package org.soltalk.soltalk_backend.domain.financial.repository;
...
public interface FinancialSummaryRepository extends JpaRepository<DailyFinancialSummary, Integer> {
    ...
    @Query(value =  "SELECT category, total_amount " +
                    ...
                    "LIMIT 5",
            nativeQuery = true)
    List<CategorySpendingSummary> findTop5Categories(@Param("userId") int userId);
    ...
    @Query(value = "SELECT d.category as category, SUM(d.total_amount) as total_amount " +
                    ...
                    "ORDER BY SUM(d.total_amount) DESC"
            , nativeQuery = true)
    List<CategorySpendingSummary> getLast7DaysSpending(@Param("userId") int userId);
