...
public class FinancialSummaryAnalyzer {
	 ...
	 * @return
	 */
    public List<CategorySpendingSummary> getTop5Categories(int userId) {
     ...
     * @return
     */
    public List<CategorySpendingSummary> getLast7DaysSpending(int userNo) {
     ...
     */
    public List<StoreSpendingSummaryResponse> getCategoryDetails(int userId) throws URISyntaxException {
        logger.info("getCategoryDetails()...userId:{}", userId);
    	// 최근 7일간 가장 지출이 많은 카테고리와, 입출금 계좌번호, user_key 받아오기
        UserCategory categoryDTO = summaryRepository.getMostSpendingCategory(userId);
